...
def _make_key_mapper(grid_size: int):
    """Build a key mapping function parameterized by grid_size."""
    gs = str(grid_size)
...
def convert(args: Args) -> None:
    ...
    assert len(model.grid_sizes) == 1, f"expected single grid size, got {model.grid_sizes}"
    grid_size = model.grid_sizes[0]
    bb = model.backbone
    map_key = _make_key_mapper(grid_size)
