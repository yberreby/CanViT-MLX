...
3. Built pure MLX reimplementation (canvit_mlx/ package)
...
- Standardizers are grid-size-specific. Grid size stored in `CanViTConfig.std_grid_size` (written to JSON metadata by convert.py), not hardcoded.
...
- `convert.py` — outputs to `weights/{repo_name}.safetensors` (derived from HF repo URL)

### Tooling
- `just` default rule: lint (ruff) → typecheck (basedpyright) → enforce no-canvit-dep → test (pytest)
- `pytest-cov` enabled by default — 100% coverage (316 stmts)
...
### Clean weight loading (DONE)
Moved ALL key remapping into `convert.py`.
- `convert.py` outputs checkpoint with MLX-native key names (244 tensors, 18 skipped)
- `load_canvit()` is trivial: `safe_open → CanViTConfig(**mc) → model.load_weights()`, zero key surgery
- convert.py now verifies correctness (PT vs MLX forward pass) after every conversion
- canvit_mlx stands on its own: clean, stable checkpoint format

### TODO: HF Hub integration
