...
Moved ALL key remapping (`_map_key()`) from `canvit_mlx/weights/` into `convert.py`.
- `convert.py` now outputs checkpoint with MLX-native key names (247 tensors, 15 skipped)
- `load_canvit()` is trivial: `safe_open → model.load_weights()`, zero key surgery
- The .safetensors format changed (breaking), regenerated by `uv run python convert.py` in ~5s
